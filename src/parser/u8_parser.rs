...
use crate::token::Value::{One, Zero};
    ...
    loop {
       state = state.next_state(tokens_iter);
        match state {
            ...
            TokensUnexpectedEnd(expected_tokens) => return Err(Parser(TokensUnexpectedEnd { parsed_type: U8, current_value_slice: &slice_from_value_start[..tokens_iter.peek().expect("BUG: 'tokens_iter' should have at least one token.").0], expected_tokens })),
            UnexpectedToken(expected_tokens) => return Err(Parser(UnexpectedToken { parsed_type: U8, current_value_slice: slice_from_value_start, expected_tokens })),
            _ => {},
        }
    }
}
...
impl States {
    fn next_state(self, tokens: &mut Peekable<Enumerate<Iter<Token>>>) -> Self {
        match self {
            ...
            End(value) => panic!("BUG: The `next_state` method should never be called on the `End` state. 'value': '{value}'."),
            TokensUnexpectedEnd(expected_tokens) => panic!("BUG: The `next_state` method should never be called on the `TokensUnexpectedEnd` state. 'expected_tokens': '{expected_tokens:?}'."),
            UnexpectedToken(expected_tokens) => panic!("BUG: The `next_state` method should never be called on the `UnexpectedToken` state. 'expected_tokens': '{expected_tokens:?}'."),
        }
    }
}
...
mod tests {
    use super::*;
    use crate::token::Value::{One, StructEnd, Zero};
